...
class ProduceOutput:
    ...
    def OutputFromTraining(self,inputs_scaled,outputs_scaled,other,tag,path_output):
        ...
        try:
            ...
            out_DY = np.power(10,-instance.HyperRestore(inputs_scaled))
            full_out = np.c_[full_out,out_DY]
        ...
        try:
            ...
            out_TT = np.power(10,-instance.HyperRestore(inputs_scaled))
            full_out = np.c_[full_out,out_TT]
    ...
    def OutputNewData(self,input_dir,list_sample,path_output,cut):
        ...
        for f in list_sample: 
            ...
            #data, weight = LoopOverTrees(input_dir=input_dir,variables=variables,weight=parameters.weights[0],reweight_to_cross_section=True,list_sample=list_sample)
            if data.shape[0]==0:
            ...
            try:
                instance = HyperModel(self.model,'DY')
                output_DY = np.power(10,-instance.HyperRestore(inputs_scaled))
            ...
            try:
                instance = HyperModel(self.model,'TT')
                output_TT = np.power(10,-instance.HyperRestore(inputs_scaled))
