...
def main():
    ...
    # Start ROC curve #
    instance_ROC_MEM = Plot_ROC('weight_TT/(weight_TT+200*weight_DY)','total_weight','MEM')
    instance_ROC_DNN = Plot_ROC('output_TT/(output_TT+200*output_DY)','total_weight','DNN')
    # Loop over files #
    for f in f_valid:
    ...
    for cat in invalid_cat:
        ...
        # Start ROC curve #
        instance_ROC_MEM = Plot_ROC('weight_TT/(weight_TT+200*weight_DY)','total_weight','Invalid {} MEM'.format(cat),"weight_TT>weight_TT_err && weight_DY>weight_DY_err".format(cat))
        instance_ROC_DNN = Plot_ROC('output_TT/(output_TT+200*output_DY)','total_weight','Invalid {} DNN'.format(cat),"weight_TT>weight_TT_err && weight_DY>weight_DY_err".format(cat))
        #instance_ROC_DNN = Plot_ROC('output_TT/(output_TT+200*output_DY)','total_weight','Invalid {} DNN'.format(cat),"weight_{0}>weight_{0}_err".format(cat))
        ...
        for f in f_inv:
            ...
            if os.path.basename(f).startswith('DY'):
                ...
                try:
                    ...
                    logging.warning('Could not add DY to ROC due to "%s"'%e)
            elif os.path.basename(f).startswith('TT'):
                logging.info('\tAdded to ROC as TT')
                try:
                    instance_ROC_MEM.AddToROC(f,'tree','TT')
                    ...
                    logging.warning('Could not add TT to ROC due to "%s"'%e)
    ...
    ######################################## JEC ##############################################
    logging.info('Starting Plotting the JEC weights')
    f_JEC =  glob.glob(INPUT_JEC+'/*.root')

    if len(f_JEC) == 0:
        logging.error('Could not find the JEC weights at %s'%(INPUT_JEC))

    # Start ROC curve #
        instance_ROC_MEM = Plot_ROC('weight_TT/(weight_TT+200*weight_DY)','total_weight','JEC MEM',"weight_TT>weight_TT_err && weight_DY>weight_DY_err".format(cat))
        instance_ROC_DNN = Plot_ROC('output_TT/(output_TT+200*output_DY)','total_weight','JEC DNN',"weight_TT>weight_TT_err && weight_DY>weight_DY_err".format(cat))
    # Loop over files #
    for f in f_JEC:
        filename = f.replace(INPUT_JEC+'/','').replace('.root','')
        logging.info('Processing JEC weights from %s'%(filename))
        ...
        for name,dict_histo in config_TH1.items():
            try:
                ...
                instance = Plot_TH1(**dict_histo)
                instance.MakeHisto()
                list_histo_JEC.append(instance)
            except Exception as e:
                logging.warning('Could not plot %s due to "%s"'%(name,e))
    ...
    try:
        ...
        logging.info('Processing ROC curve for DNN') 
        instance_ROC_DNN.ProcessROC()
        MakeROCPlot([instance_ROC_MEM],os.path.join(OUTPUT_PDF,'JEC_ROC_MEM'))
        MakeROCPlot([instance_ROC_DNN],os.path.join(OUTPUT_PDF,'JEC_ROC_DNN'))
        MakeROCPlot([instance_ROC_MEM,instance_ROC_DNN],os.path.join(OUTPUT_PDF,'JEC_ROC_MEM_vs_DNN'))
    except Exception as e:
        logging.warning('Could not process ROC due to "%s"'%e)
    ...
    LoopPlotOnCanvas(os.path.join(OUTPUT_PDF,opt.model+'_JEC'),list_histo_JEC)
